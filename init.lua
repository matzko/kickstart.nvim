...
require('lazy').setup({
  ...
  'github/copilot.vim',
  {
    ...
    event = 'VeryLazy',
    lazy = false,
    version = false, -- Never set this to "*"! Never!
    opts = {
      ...
      behaviour = {
        ...
        auto_set_keymaps = false, -- Disable automatic keymap setting to avoid conflicts
      },
      -- Configure windows
      windows = {
    ...
    -- Manual keymap setup to avoid conflicts
    keys = {
      {
        '<leader>aa',
        function()
    ...
    config = function(_, opts)
      ...
      if ok then
        ...
        vim.notify('Failed to load avante_lib, trying to build...', vim.log.levels.WARN)
        vim.fn.system('cd ' .. vim.fn.stdpath 'data' .. '/lazy/avante.nvim && make')
        -- Try loading again
        local ok2, avante_lib2 = pcall(require, 'avante_lib')
        if ok2 then
          avante_lib2.load()
        end
      end

      -- Setup avante with error handling
      local ok_avante, avante = pcall(require, 'avante')
      if ok_avante then
        avante.setup(opts)
      else
        vim.notify('Failed to setup avante: ' .. tostring(avante), vim.log.levels.ERROR)
      end
    end,
    ...
    dependencies = {
      ...
      {
        -- Markdown rendering for better display
